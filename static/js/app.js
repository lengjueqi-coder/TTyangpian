    ...
    async function checkUpdate() {
        ...
        try {
            ...
            if (result.has_update) {
                ...
                showPanel('latest');
                const title = document.getElementById('update-latest-title');
                const detail = document.getElementById('update-latest-detail');
                if (result.release_status === 'local_ahead') {
                    title.textContent = '本机代码版本高于可下载版本';
                    detail.textContent = `本机 ${result.local_version}；GitHub Release ${result.remote_version}。新版安装包尚未发布。`;
                } else {
                    title.textContent = `已是最新版本 ${result.local_version || '1.0.0'}`;
                    detail.textContent = result.remote_version ? `GitHub Release ${result.remote_version}` : '';
                }
